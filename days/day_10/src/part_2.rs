pub fn solution(input: &str) -> u32 {
    ...
    for row in 0..grid.tiles.len() {
        let tile_row = &grid.tiles[row];
        for column in 0..tile_row.len() {
            let point = (row as i32, column as i32);

            if pipe_loop.contains(&(row as i32, column as i32)) {
                continue;
            };

            if is_point_inside_polygon(&point, &pipe_loop) {
                count += 1;
            }
        }
    }
...
///
fn is_point_inside_polygon(point: &(i32, i32), polygon: &Vec<(i32, i32)>) -> bool {
    let x = point.0;
    let y = point.1;
