...
impl Report {
    fn extrapolate(&mut self) {
        ...
        for index in (0..self.history.len() - 1).rev() {
            ...
            let prediction_right = *prediction.0.first().expect("Last prediction number");
            let prediction_bottom = *future_prediction.0.first().expect("Last prediction number");

            self.history
                .get_mut(index)
                .expect("Last prediction")
                .0
                .insert(0, prediction_right - prediction_bottom);
        }
    }
}
